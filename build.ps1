# Build GHCraftPad.dll and (optionally) deploy it into Green Hell's BepInEx plugins folder.
#
...
$outDll  = Join-Path $outDir 'GHCraftPad.dll'
...
Write-Host "Compiling GHCraftPad..." -ForegroundColor Cyan
& $csc $argList.ToArray()
...
$dest = Join-Path $GameDir 'BepInEx\plugins\GHCraftPad'
...
try {
    Copy-Item $outDll (Join-Path $dest 'GHCraftPad.dll') -Force -ErrorAction Stop
} catch {
    Write-Host ""
...
Write-Host "" 
Write-Host "Open the crafting table: the pad lists what you can make from your pack and the boxes around you." -ForegroundColor Yellow
Write-Host ""
